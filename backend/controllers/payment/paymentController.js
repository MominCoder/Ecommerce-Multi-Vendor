...
class paymentController {
  ...
  get_seller_payemt_details = async (req, res) => {
    ...
    try {
      ...
      const pendingWithdrows = await withdrawRequest.find({
        $and: [
          {
            sellerId: {
              $eq: sellerId,
      ...
      const successWithdrows = await withdrawRequest.find({
        $and: [
          {
            sellerId: {
              $eq: sellerId,
  ...
  withdrowal_request = async (req, res) => {
    ...
    try {
      const withdrowal = await withdrawRequest.create({
        sellerId,
  ...
  get_payment_request = async (req, res) => {
    try {
      const withdrowalRequest = await withdrawRequest.find({
        status: "pending",
      });
